...
- [x] EDA and feature engineering pipeline
- [x] Baseline and advanced model training scripts
- [x] Evaluation plots and reports
- [x] SHAP explainability analysis
- [x] Real-time scoring API with FastAPI
...
Built an end-to-end listener retention ML pipeline for an audio streaming platform use case, using calibrated synthetic behavioral data validated against the Last.fm HetRec 2011 public dataset. Designed an at-risk listener target, trained and tuned Random Forest models with Optuna, added threshold calibration, SHAP explainability, FastAPI inference, CI workflow, and imbalance-aware evaluation against dummy baselines. Final at-risk model achieved ROC-AUC 0.9803, PR-AUC 0.9715, F1 0.9600, and balanced accuracy 0.9715.
...
- Current data is synthetic, so production performance must be revalidated with real user behavior data.
- The target definition is rule-assisted and may need business calibration for a real streaming platform.
- XGBoost and LightGBM may require OpenMP runtime on macOS.
- Batch jobs run locally unless deployed with a proper queue/worker system.
